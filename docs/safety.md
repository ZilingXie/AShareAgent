...
- LLM 只做盘前结构化分析辅助和策略复盘解释，不直接生成交易指令。
- backtest、strategy-evaluate 和 strategy-insight 内部触发的多窗口评估强制使用 mock LLM，避免多日回放消耗真实 API；LLM 不参与 SignalEngine 或 RiskManager 决策。
- strategy-insight 的真实 LLM 只允许输出结构化 hypotheses JSON；JSON 解析失败、缺字段或建议越界必须显式失败或标记 `rejected_by_policy`，不能自动修改 `configs/strategy_params.yml`。
