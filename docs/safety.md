...
当前状态：已落地 Mock pipeline、真实数据入口、DataQualityAgent 质量门禁、DataReliabilityAgent 运行可靠性报告、结构化交易日历、daily-run、完整 PaperTrader 模拟持仓生命周期、策略参数版本审计、多日 backtest 回放和只读观察台。本文记录项目必须长期遵守的交易和数据安全边界。
